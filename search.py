...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    frontier.push([problem.getStartState(), [], startHeuristic], startHeuristic)
    expanded = []
    while not frontier.isEmpty():
        node = frontier.pop()
        if problem.isGoalState(node[0]):
        ...
        if node[0] not in expanded:
            ...
            for child in problem.expand(node[0]):
                path = node[1] + [child[1]]
                newP = heuristic(child[0], problem) + child[2]
                frontier.push([child[0], path, child[2] + node[2]], newP + node[2])
